from datetime import datetime, timedelta
from airflow import DAG
from airflow.operators.empty import EmptyOperator
from airflow.operators.python import PythonOperator


TAGS = ["PtyhonDataFlow"]
DAG_ID = "dependencia_dag"
DAG_DESCRIPCION = """ Probando dags de airflow """
DAG_SCHEDULE  = "0 9 * * *"
default_args = {
    "start_date": datetime (2022,1,1)
}
retries = 4
retry_delay = timedelta(minutes=5)

def execute_taks ():
    print("probando")


dag = DAG (
    dag_id = DAG_ID,
    description= DAG_DESCRIPCION,
    catchup= False, #hace que el dag se ponga al día
    schedule_interval=DAG_SCHEDULE,
    max_active_runs=1, #maxima canticdad de ejecuciones
    dagrun_timeout=200000,
    default_args=default_args,
    tags=TAGS
)



# ahora se crean las tareas asociadas a ese DAG


with dag as dag:
    
    start_task = EmptyOperator (
        task_id="start_task"
    )


    end_task = EmptyOperator (
        task_id="end_task"
    )


    first_task = PythonOperator (
        task_id = "first_task",
        python_callable=execute_taks,
        retries= retries,
        retry_delay = retry_delay
    )


    second_task = PythonOperator (
            task_id = "second_task",
            python_callable=execute_taks,
            retries= retries,
            retry_delay = retry_delay
        )
    
    third_task = PythonOperator (
            task_id = "third_task",
            python_callable=execute_taks,
            retries= retries,
            retry_delay = retry_delay
        )

    fourth_task = PythonOperator (
            task_id = "fourth_task",
            python_callable=execute_taks,
            retries= retries,
            retry_delay = retry_delay
        )

    fith_task = PythonOperator (
            task_id = "fith_task",
            python_callable=execute_taks,
            retries= retries,
            retry_delay = retry_delay
        )
# establecemos dependencias de tareas


# deben acabar las dos tareas en paralelo
# para que se ejecute end taks
# start_task >> [first_task, second_task, third_task] >> end_task


start_task >> [first_task, second_task] >> third_task
third_task >> [fourth_task, fith_task]